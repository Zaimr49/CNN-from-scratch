...
class TestForward:
    ...
    def test_five(self):
        """Tests the forward pass function for correctness given 1x1 convolution"""

        input_shape = (1, 2, 3, 3)
        num_filters = 1
        conv_layer = ConvolutionLayer(5, num_filters, 1, stride=1)
        conv_layer.weights = np.array([[[[1]]]])
        conv_layer.bias = np.array([0])

        input_array = np.arange(18).reshape(*input_shape)

        expected_output = np.array([[9, 11, 13], [15, 17, 19], [21, 23, 25]]).reshape(
            1, 1, 3, 3
        )
